package com.wg.edfwriteedftest.EDFlib;/*
*****************************************************************************
*
*
*****************************************************************************
...
private final RandomAccessFile file_in;
...
/********************* PATIENTNAME *********************************************/
  ...
  patient = new String(hdr, 8, 80, StandardCharsets.US_ASCII);

/********************* RECORDING *********************************************/

  recording = new String(hdr, 88, 80, StandardCharsets.US_ASCII);
...
/********************* RESERVED FIELD *************************************/
  ...
  reserved = new String(hdr, 192, 44, StandardCharsets.US_ASCII);
  ...
  if(bdf != 0)
  {
    ...
    if(reserved.startsWith("BDF+C"))
    {
      bdfplus = 1;
      filetype = EDFLIB_FILETYPE_BDFPLUS;
    }

    if(reserved.startsWith("BDF+D"))
    {
      bdfplus = 1;
      discontinuous = 1;
      filetype = EDFLIB_FILETYPE_BDFPLUS;
...
/**************************** LABELS *************************************/
  ...
  {
    param_label[i] = tmp_str = new String(hdr, 256 + (i * 16), 16, StandardCharsets.US_ASCII);

    if(edfplus != 0)
    {
      if(param_label[i].startsWith("EDF Annotations "))
      {
    ...
    if(bdfplus != 0)
    {
      if(param_label[i].startsWith("BDF Annotations "))
      {
        annot_ch[nr_annot_chns] = i;
        nr_annot_chns++;
        param_annotation[i] = 1;
...
/**************************** TRANSDUCER TYPES *************************************/
  ...
  for(i=0; i<edfsignals; i++)
  {
    ...
    param_transducer[i] = tmp_str = new String(hdr, 256 + (edfsignals * 16) + (i * 80), 80, StandardCharsets.US_ASCII);
  }
...
/**************************** PHYSICAL DIMENSIONS *************************************/
  ...
  for(i=0; i<edfsignals; i++)
  {
    param_physdimension[i] = tmp_str = new String(hdr, 256 + (edfsignals * 96) + (i * 8), 8, StandardCharsets.US_ASCII);
  }
...
/**************************** PREFILTER FIELDS *************************************/
  ...
  for(i=0; i<edfsignals; i++)
  {
    ...
    param_prefilter[i] = tmp_str = new String(hdr, 256 + (edfsignals * 136) + (i * 80), 80, StandardCharsets.US_ASCII);
  }
...
/**************************** RESERVED FIELDS *************************************/
  ...
  for(i=0; i<edfsignals; i++)
  {
    param_reserved[i] = tmp_str = new String(hdr, 256 + (edfsignals * 224) + (i * 32), 32, StandardCharsets.US_ASCII);
  }
...
/********************* EDF+ PATIENTNAME *********************************************/
  ...
  if((edfplus != 0) || (bdfplus != 0))
  {
    ...
    else
    {
      ...
      plus_patientcode = tmp_str = new String(str128, 0, i, StandardCharsets.US_ASCII);
      p += i + 1;
    }
    ...
    else
    {
      ...
      str128[6] = ' ';
      p += i + 1;
      plus_birthdate = tmp_str = new String(str128, 0, 11, StandardCharsets.US_ASCII);
    }
    ...
    p += i + 1;
    plus_patient_name = tmp_str = new String(str128, 0, i, StandardCharsets.US_ASCII);
    ...
    p += i + 1;
    plus_patient_additional = tmp_str = new String(str128, 0, i, StandardCharsets.US_ASCII);
  }
...
/********************* EDF+ RECORDINGFIELD *********************************************/
  ...
  if((edfplus != 0) || (bdfplus != 0))
  {
    ...
    str128[6] = ' ';
    p += i + 1;
    plus_startdate = tmp_str = new String(str128, 0, i, StandardCharsets.US_ASCII);
    ...
    else
    {
      ...
      p += i + 1;
      plus_admincode = tmp_str = new String(str128, 0, i, StandardCharsets.US_ASCII);
    }
    ...
    else
    {
      ...
      p += i + 1;
      plus_technician = tmp_str = new String(str128, 0, i, StandardCharsets.US_ASCII);
    }
    ...
    else
    {
      ...
      p += i + 1;
      plus_equipment = tmp_str = new String(str128, 0, i, StandardCharsets.US_ASCII);
    }
    ...
    p += i + 1;
    plus_recording_additional = tmp_str = new String(str128, 0, i, StandardCharsets.US_ASCII);
  }
        ...
        if((scratchpad[n] == 20) || (scratchpad[n] == 21))
        {
          ...
          if((scratchpad[n] == 20) && (onset != 0) && (duration_start == 0))
          {
            if((r != 0) || (annots_in_record != 0))
            {
              ...
              new_annotation.description = new String(scratchpad, 0, n, StandardCharsets.UTF_8);
